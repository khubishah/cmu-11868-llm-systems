...
__global__ void ker_attn_softmax(T *inp, const T *attn_mask, int from_len,
  ...
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
    ...
    // thread local max
    // BEGIN ASSIGN4_1_1
    // Compute thread-local max using fmaxf() similar to lt32 version
    float val[token_per_reduce][ele_per_thread];
    float l_max[token_per_reduce];
    for (int i = 0; i < token_per_reduce; i++) {
      l_max[i] = REDUCE_FLOAT_INF_NEG;
    ...
    // thread local sum
    // BEGIN ASSIGN4_1_1
    // Compute exp and thread-local sum using __expf()
    float l_sum[token_per_reduce];
    for (int i = 0; i < token_per_reduce; i++) {
      l_sum[i] = 0.f;
      for (int j = 0; j < ele_per_thread; j++) {
    ...
    /* step 3. compute final result */
    // BEGIN ASSIGN4_1_1
    // Normalize using __fdividef() and store with BlockStore
    for (int i = 0; i < token_per_reduce && (token_id + i) < from_len; i++) {
      for (int j = 0; j < ele_per_thread; j++) {
...
__global__ void ker_attn_softmax_bw(T *grad, const T *inp, int softmax_length) {
  ...
  int base_offset = batch_idx * softmax_length;
  
  float sum = 0.0;
  float grad_vals[ITERATIONS];
  float inp_vals[ITERATIONS];
  
  // Load all data in one pass with coalesced access
  #pragma unroll
  for (int iter = 0; iter < ITERATIONS; ++iter) {
    int idx = threadIdx.x + iter * WARP_SIZE;
    if (idx < softmax_length) {
      grad_vals[iter] = (float)grad[base_offset + idx];
      inp_vals[iter] = (float)inp[base_offset + idx];
      sum += grad_vals[iter] * inp_vals[iter];
    } else {
      grad_vals[iter] = 0.0f;
      inp_vals[iter] = 0.0f;
    }
  ...
  // Write back in one pass
  #pragma unroll
  for (int iter = 0; iter < ITERATIONS; ++iter) {
    int idx = threadIdx.x + iter * WARP_SIZE;
    if (idx < softmax_length) {
      grad[base_offset + idx] = (T)(inp_vals[iter] * (grad_vals[iter] - sum));
    }
  }
}
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  // Optimize for better occupancy - back to 4 for better cache usage
  const int warps_per_block = 4;
  dim3 grid_dim((rows + warps_per_block - 1) / warps_per_block);
  dim3 block_dim(WARP_SIZE, warps_per_block);
  // BEGIN ASSIGN4_1_2
  
  // No memory allocation needed - data is already on GPU!
  // Work directly with GPU pointers for maximum performance
  
  // Launch kernel directly on GPU data with more aggressive ITERATIONS
  if (softmax_len <= 16) {
    ker_attn_softmax_bw<float, 1><<<grid_dim, block_dim, 0, stream>>>(
        out_grad, soft_inp, softmax_len);
  } else if (softmax_len <= 32) {
    ker_attn_softmax_bw<float, 1><<<grid_dim, block_dim, 0, stream>>>(
        out_grad, soft_inp, softmax_len);
  } else if (softmax_len <= 64) {
    ker_attn_softmax_bw<float, 2><<<grid_dim, block_dim, 0, stream>>>(
        out_grad, soft_inp, softmax_len);
  } else if (softmax_len <= 96) {
    ker_attn_softmax_bw<float, 3><<<grid_dim, block_dim, 0, stream>>>(
        out_grad, soft_inp, softmax_len);
  } else if (softmax_len <= 128) {
    ker_attn_softmax_bw<float, 4><<<grid_dim, block_dim, 0, stream>>>(
        out_grad, soft_inp, softmax_len);
  } else if (softmax_len <= 192) {
  ...
  // No memory copies needed - result is already in GPU memory!
  // No synchronization needed - stream handles async execution
  
  // Check CUDA execution (optional for debugging)
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess) {
